...
public class Authentification {
	...
	private void initialize() {
		...
		signin.addActionListener(new ActionListener() {
			...
			String sql = "select count (*) from Personnes where loginconnexion = '"+username+"' and motdepasse = '"+password+"'" ;
			...
			if ( acces == 0 )
			{
				...
				JOptionPane.showMessageDialog(null, "Connexion Reussite");
				 sql = "select statut from Personnes where loginconnexion = '"+username+"' and motdepasse = '"+password+"'" ;
				 String statutlogin = AccesJDBC.trouverNom(sql);
				 
				 Menu window = new Menu(statutlogin);
